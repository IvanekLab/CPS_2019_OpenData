...
to setup
  ...
  with-local-randomness[ ;we interrupt the global seed during setup so that we get different combinations of parameter values when sampling the parameter space
    ...
    print (word "random seed:" seed-me)
  import-network
...
to calculate-sample-prevalence ; here is where you need to define global outputs based on facility, agents, and sampling data - these are collected from sample function so need to match there
  ...
  ifelse curtain-samples != 0 [  set curtain-prev (curtain-prev / curtain-samples )] [set curtain-prev 0]
  ...
  ifelse metal-fc-samples != 0 [  set metal-fc-prev (metal-fc-prev / metal-fc-samples )] [set metal-fc-prev 0]
  ...
  ifelse pillar-samples != 0 [  set pillar-prev (pillar-prev / pillar-samples )] [set pillar-prev 0]
  ...
  ifelse wheels-samples != 0 [  set wheels-prev (wheels-prev / wheels-samples )] [set wheels-prev 0]
...
; are there times when the room is empty?
 ...
 ; these things occur during pre-op inspection
  ...
  if event = "production-1" [
    ...
    ;print (word "production-time:" production-time)
    if production-time = 0 and sampling-time = "first-hour" and start-sampling? [sample]
...
to sample ; will need modification depending on historical sampling practice and data to be collected
  ...
  ask possible-z-sites  [
        ...
        if z-equipment = "curtain" [set curtain-samples curtain-samples + 1]
        ...
        ;if z-equipment = "floor" [set floor-samples floor-samples + 1]
        if z-equipment = "frame" [set frame-samples frame-samples + 1]
        if z-equipment = "metal-fc" [set metal-fc-samples metal-fc-samples + 1]
        ...
        if z-equipment = "pillar" [set pillar-samples pillar-samples + 1]
        if z-equipment = "table" [set table-samples table-samples + 1]
        if z-equipment = "trash" [set trash-samples trash-samples + 1]
        if z-equipment = "wheels" [set wheels-samples wheels-samples + 1]
        ...
        if result = 1
          [set z-sample-prev z-sample-prev + 1
           ...
           if z-equipment = "curtain" [set curtain-prev curtain-prev + 1]
...
to Scenario-1
  ...
  ;set possible-p-sites n-of 15 patches with [pcolor != 0] ;comment out this line if not modeling floor
end

to Scenario-2
  set zone1-sample 35
  set zone2-sample 35
  set zone3-sample 35 ; change to 35 if not sampling floor
  set possible-z-sites (turtle-set (n-of zone1-sample zones with [ z-category = 1])
  (n-of zone2-sample zones with [ z-category = 2]) (n-of zone3-sample zones with [z-category = 3]))
  ;set possible-p-sites n-of 15 patches with [pcolor != 0] ;comment out this line if not modeling floor
end
...
to Scenario-3
  set possible-z-sites (turtle-set (n-of 105 zones))
  ;set possible-p-sites n-of 15 patches with [pcolor != 0]
end

to Scenario-4
  set zone1-sample 0
  set zone2-sample 0
  set zone3-sample 105
  set possible-z-sites (turtle-set (n-of zone1-sample zones with [ z-category = 1])
  (n-of zone2-sample zones with [ z-category = 2]) (n-of zone3-sample zones with [z-category = 3]) )
  ;set possible-p-sites n-of 15 patches with [pcolor != 0]
end
...
<experiments>
  ...
  <experiment name="validation_prevalence" repetitions="1000" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>p11</metric>
    <metric>p12</metric>
  ...
  <experiment name="Scenario_trials" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>p11</metric>
    <metric>p12</metric>
